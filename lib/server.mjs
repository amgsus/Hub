...
export class Hub extends EventEmitter {
    ...
    #acceptConnection(connection) {
        ...
        this.#dictionary.addSubscriber(client, client.notifyExpression);
    }
    ...
    #onClientClose(client) {
        ...
        // FIXME: this.#unregisterClientsRPCs(client, `${client.idString} disconnects`);
        arrayDeleteItem(this.#clients, client);
    ...
    #onClientStore(client, keyName, value, ts) {
        ...
        if (!entry) {
            entry = this.#dictionary.createValue(keyName);
            entry.subscribers = this.#clients.filter(function (client2) {
                return client2.notifyExpression.test(keyName);
            });
            log.trace(`Client ${client.idString} has created key: ${entry.path}`);
            log.trace(`Total keys stored: ${this.#dictionary.length}`);
        }
        if (ts > entry.ts) {
            // entry.value = value;
            // entry.ts = ts;
            this.#dictionary.updateValue(keyName, value, ts); // FIXME
        }
    ...
    static #getUniqueClientID() { // FIXME: Make global counter across all server sockets. Done: made static.
        let id;
        do {
            id = Hub.#nextClientID;
            Hub.#nextClientID++; // FIXME: Limiting value?
        } while (Hub.#clientMap[`${id}`]);
        return id;
    }
