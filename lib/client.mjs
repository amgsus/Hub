...
export class HubConnectionManager extends EventEmitter {
    ...
    constructor(server) {
        ...
        if (!(server instanceof Hub)) {
            throw new Error(`server must be instance of Hub`);
        }
    ...
    filterClientsByNotificationMask(keyName) { return []; }

    getClientsByServer(server) { return []; }
    ...
    acceptConnection(socket) {
        ...
        let client = new ClientHandler(this.#server, clientID, socket,
            this.#defaultNotificationMask, "none", this.#defaultClientOpts); // FIXME
    ...
    #getUniqueClientID() {
        ...
        do {
            ...
            this.#nextClientID++; // FIXME: Limiting value?
